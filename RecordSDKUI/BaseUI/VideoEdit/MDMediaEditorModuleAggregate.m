...
//#import "MDDocumentRenderer.h"
//#import "BBGPUImageMediaEditorOutputFilter.h"
...
//播放相关
//@property (nonatomic, strong) MDRecordPlayerViewController          *playerViewController;
//@property (nonatomic, strong) id                                    periodicTimeObserver;
@property (nonatomic, strong) NSURL                                 *soundPitchURL;
//导出相关
//@property (nonatomic, strong) MDDocumentRenderer                    *documentRenderer;
@property (nonatomic, strong) MDDocument                            *document;
//@property (nonatomic, strong) id<MDDocumentExportSession>           exportSession; //导出会话
...
//@property (nonatomic, strong) MDVideoEditorContext *context;
//@property (nonatomic, strong) MDVideoRenderPipline *renderPipline;
//
//@property (nonatomic, strong) id<MDVideoExportProtocol> export;
...
//    self.adapter.specialEffectsFilter.timeRange = videoTimeRange;
    
//    self.context = [[MDVideoEditorContext alloc] init];
//    __weak typeof(self) weakself = self;
//    self.context.playToEndTime = ^(AVPlayer * _Nonnull player) {
//        __strong typeof(self) self = weakself;
//        if (![self.speedVaryVc isViewVisible] && self.exportTask == nil && !self.specialEffectsVc.isShow) {
//            [player seekToTime:kCMTimeZero toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero];
//            [self play];
...
//    };
//
//    self.context.playerPeriodicTimeCallback = ^(CMTime time) {
//        __strong typeof(self) self = weakself;
//        if ([self.musicSelectPicker viewIsShowing]) {
//            [self.musicSelectPicker periodicTimeCallback:time];
//        }
//    };
//
//    self.renderPipline = [[MDVideoRenderPipline alloc] init];
//    self.renderPipline.detector.canUseBodyThinSetting = YES;
//    self.renderPipline.renderFilter.specialFilterGroup.timeRange = videoTimeRange;
//    self.context.filter = self.renderPipline;
//    self.renderPipline.renderFilter.slidingOffset = 1.0;
//    self.renderPipline.renderFilter.canUseAIBeautySetting = YES;
...
{
//    if ([MDContext currentUser].isVideoRecording) {
//        [self.context pause];
...
//    self.renderPipline.renderFilter.overlayImage = nil;
...
//    [composition insertTimeRange:CMTimeRangeMake(kCMTimeZero, self.document.assetToBeProcessed.duration)
//                         ofAsset:self.document.assetToBeProcessed
//                          atTime:kCMTimeZero error:nil];
//
//    MDVideoAssetProcessBuilder *builder = self.document.builder;
//    id<MDAssetProcessProtocol> assetProcess = [builder buildAssetProcess];
//    AVAsset *processedAsset = [assetProcess processAsset:composition error:NULL];
//    AVAudioMix *audioMix = [builder buildAudioMix];
//    [self.context updateAssetUsingAsset:processedAsset audioMix:audioMix];
...
//    MDVideoAssetProcessBuilder *builder = self.document.builder;
//    AVAudioMix *audioMix = [builder buildAudioMix];
//    [self.context udpateAudioMix:audioMix];
    [self updateBuilderSetting];
...
- (void)activatePlayer
{
//    self.context.playerViewController.view.frame = self.viewController.view.bounds;
...
//            NSInteger currentFilterIndex = index;
//            NSInteger tempFitlerIndex;
...
//            MDRecordFilter *filterB = [self.filters objectAtIndex:tempFitlerIndex defaultValue:nil];
//            self.renderPipline.renderFilter.slidingMDFilterB = filterB;
//            MDRecordFilter *filterA = [self.filters objectAtIndex:currentFilterIndex defaultValue:nil];
//            self.renderPipline.renderFilter.slidingMDFilterA = filterA;
//            [self.adapter configFilterA:filterA configFilterB:filterB offset:1.0];
...
//    NSMutableDictionary *mDict = [NSMutableDictionary dictionary];
    ...
    [self.beautySettingDict enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        // [[MDContext beautySettingDataManager] canUseAIBeautySetting]
        if (YES) {
//            CGFloat realValue = [[MDContext beautySettingDataManager] realValueWithIndex:[obj integerValue] beautySettingTypeStr:key];
//            CGFloat realValue = [obj integerValue] / 100.0f; //
...
//            [mDict setFloat:realValue forKey:key];
//        }
    }];
...
//                [weakSelf.delegate didEndReverseVideoOpertion];
//                if (!isCancelled) {
//                    [[MDRecordContext sharedIndicate] showWarningInView:weakSelf.viewController.view withText:@"特效滤镜视频处理失败" timeOut:1.5];
//                }
//            });
//            weakSelf.reserveOperation = nil;
...
- (void)configExportVideoSetting {
    ...
    NSInteger clarityStrategy = 1;
    if (clarityStrategy==1 && presentationSize.width*presentationSize.height > 1280*720) {
...
//                          atTime:kCMTimeZero error:nil];
//
//    id<MDAssetPorcessBuilder> builder = self.document.builder;
//    id<MDAssetProcessProtocol> assetProcess = [builder buildAssetProcess];
//    AVAsset *processedAsset = [assetProcess processAsset:composition error:NULL];
//    AVAudioMix *audioMix = [(MDVideoAssetProcessBuilder *)builder buildAudioMix];
//    id sourceFormatHint = [(MDVideoAssetProcessBuilder *)builder audioSourceHint];
//
//    MDVideoExport *export = [[MDVideoExport alloc] init];
//    export.asset = [processedAsset copy];
//    export.audioMix = audioMix;
//
//    export.presentationSize = presentationSize;
//    export.targetBitRate = sourceBitRate;
//    export.videoPerferredTransform = self.document.videoPreferredTransform;
//    export.audioFormatHint = (__bridge CMFormatDescriptionRef)sourceFormatHint;
//
//    BOOL needFilterEffect = self.stickers.count > 0 || self.document.customOverlay || [self needBeautySettingFilter] || self.document.hasPictureSpecialEffects;
//    if (needFilterEffect) {
//        export.imageProcess = self.renderPipline;
//    }
//
//    return export;
}
...
//    MDVideoExport *export = [self createExport];
//    self.export = export;
//    //视频导出到指的目录下
//    __weak typeof(self) weakSelf = self;
//    self.exportTask = [export exportToURL:url progressHandler:^(double progress) {
...
- (BOOL)checkHasEditVideoPicture {
    ...
    if (self.stickers.count || self.textStickers.count) {
        result = YES;
    }
    
    if ([self.document timeRangeMappingEffects].count) {
        result = YES;
    }
...
//    self.renderPipline.renderFilter.beautyConfiguration = config;
