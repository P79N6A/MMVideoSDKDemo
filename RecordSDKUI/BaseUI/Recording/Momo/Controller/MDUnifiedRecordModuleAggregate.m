...
- (void)setupCameraSourceHandlerWithMaxDuration:(NSTimeInterval)maxDuration minDuration:(NSTimeInterval)minDuration contentView:(UIView *)contentView devicePosition:(AVCaptureDevicePosition)position {
    ...
    [self.recordHandler setCaptureStillImageHandler:^(UIImage *stillImage) {
        dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [self.recordHandler setFaceFetureHandler:_faceFetureHandler];
#endif
...
-(void)setFaceFetureHandler:(void (^)(CVPixelBufferRef))faceFetureHandler {
    ...
    _faceFetureHandler = faceFetureHandler;
    [self.recordHandler setFaceFetureHandler:faceFetureHandler];
}
#endif
...
//        [self.recordHandler setBeautyBigEye:bigEyeFactor];
//        [self.recordHandler setBeautyThinFace:thinFaceFactor];
//        [self.recordHandler setBeautySkinWhite: skinWhitenFactor];
//        [self.recordHandler setBeautySkinSmooth:skinSmoothFactor];
//        [self.recordHandler setBeautyThinBody:thinBodyFactor];
//        [self.recordHandler setBeautyLongLeg:longLegFactor];
        ...
        [self.recordHandler setBeautyBigEye:[self.realBeautySettingDict floatForKey:MDBeautySettingsEyesEnhancementAmountKey defaultValue:0.0f]];
        [self.recordHandler setBeautyThinFace:[self.realBeautySettingDict floatForKey:MDBeautySettingsFaceThinningAmountKey defaultValue:0.0f]];
        [self.recordHandler setBeautySkinWhite: [self.realBeautySettingDict floatForKey:MDBeautySettingsSkinWhitenAmountKey defaultValue:0.0f]];
        [self.recordHandler setBeautySkinSmooth:[self.realBeautySettingDict floatForKey:MDBeautySettingsSkinSmoothingAmountKey defaultValue:0.0f]];
        [self.recordHandler setBeautyLongLeg:[self.realBeautySettingDict floatForKey:MDBeautySettingsLongLegAmountKey defaultValue:0.0f]];
        [self.recordHandler setBeautyThinBody:[self.realBeautySettingDict floatForKey:MDBeautySettingsThinBodyAmountKey defaultValue:0.0f]];
...
//        else {
//            CGFloat realValue = [weakSelf realValueWithIndex:[obj integerValue] beautySettingTypeStr:key];
//            [weakSelf.realBeautySettingDict setFloat:realValue forKey:key];
//        }
    }];
}
...
- (void)activateAutoMusicWithMusicItem:(MDMusicCollectionItem *)musicItem needSameStyle:(BOOL)needSameStyle
{
    if (!self.hasUsePreMusic && !self.currentSelectMusicItem && [musicItem.musicVo.musicID isNotEmpty]) {
        ...
        if (!resourceExist) { //下载配置的音乐
            ...
            [[MDNewMusicDownloadManager shared] download:musicItem.musicVo completion:^(MDMusicBVO *bvo, NSURL * fileUrl, BOOL success) {
